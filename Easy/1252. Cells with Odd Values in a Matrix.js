...
//* Array.from | for of | nested for loop
...
//! Array.from | for | forEach | no nested loop
const oddCells = (n, m, indices) => {
    ...
    // update col
		for (let i = 0; i < n; i++) {
      matrix[i][c]++;
      // if cell becomes odd, increment by 1, else decrement by 1
			count += matrix[i][c] % 2 ? 1 : -1;
		}
		// update row
		for (let i = 0; i < m; i++) {
			matrix[r][i]++;
			count += matrix[r][i] % 2 ? 1 : -1;
		}
	}
